...
### 2026-06-03 — Багфикс: видео-обои не отображались на рабочем столе после «Применить»
- **Симптом:** пользователь выбирал видео, нажимал «Применить как обои» (или «Применить» в библиотеке), но на рабочем столе ничего не появлялось — ни видео, ни чёрного экрана поверх иконок. Превью в приложении при этом работало нормально.
- **Причина — комбинация трёх факторов:**
  1. **URL-энкодинг путей.** `FileDialog.selectedFile` возвращает `url`, и `toString()` выдаёт процент-энкодинг: пробелы → `%20`, кириллица → `%D0%90…`. В `HomePage.qml` снимался только префикс `file:///`, без `decodeURIComponent()`. Для пути `C:/Users/Александр/Видео/обои.mp4` в `Settings.currentWallpaper` попадало `C:/Users/%D0%90%D0%BB%D0%B5%D0%BA%D1%81%D0%B0%D0%BD%D0%B4%D1%80/…`. `QFile::exists()` в `Library.addFile()` не находил файл (тост «Файл не найден» — но пользователь мог не заметить), а mpv при `loadfile` не открывал энкоженный путь. Для путей без пробелов и не-ASCII (латиница в корне диска) этот баг не проявлялся.
  2. **`Qt.Tool` на хост-окне.** Хост-окно обоев создавалось с `flags: Qt.FramelessWindowHint | Qt.Tool`. Qt трактовал его как «transient tool palette» — привязывал к `ApplicationWindow`, скрывал при потере фокуса главным окном, и конфликтовал с `SetParent(WorkerW)` на Win32-уровне: Qt-уровень считал окно «tool-window» и управлял его видимостью через призму родителя-приложения, а Win32-уровень — child-window у WorkerW. Результат: окно либо не показывалось вовсе, либо исчезало при сворачивании главного окна.
  3. **Стили Win32 до SetParent, без `SWP_FRAMECHANGED`.** `SetWindowLongPtrW(GWL_STYLE, …WS_CHILD…)` вызывался **до** `SetParent`. `WS_CHILD` на окне без родителя — undefined behavior по MSDN: окно становится невидимым. `SetParent` должен был это исправить, но без `SetWindowPos(SWP_FRAMECHANGED)` новые стили не применялись к non-client area. Плюс не было постоянного «насоса» для render-loop: после `SetParent` хост-окно переставало получать `QExposeEvent` от платформы (child-окна WorkerW не проходят через top-level expose flow), и scene graph замирал — `MpvRenderer::render()` не вызывался, кадры mpv не отображались. `m_renderKickTimer` в MpvObject помогал только до создания render-контекста, потом останавливался.
- **Правка `src/qml/pages/HomePage.qml`:** после снятия `file:///` добавлен `decodeURIComponent(path)`. Пути с пробелами, кириллицей и любым не-ASCII теперь корректно доходят и до `Library.addFile()`, и до mpv `loadfile`.
- **Правка `src/qml/Main.qml`:** убран `Qt.Tool` из flags хост-окна. Оставлен только `Qt.FramelessWindowHint`. Qt больше не управляет видимостью окна как tool-palette; после `SetParent` окно полноценно живёт в слое WorkerW.
- **Правка `src/core/WallpaperEngine.cpp`:**
  - `SetParent(hwnd, workerW)` теперь вызывается **до** `SetWindowLongPtrW(GWL_STYLE, WS_CHILD)`. MSDN предписывает: сначала репаратрить, потом менять стиль на `WS_CHILD`.
  - Добавлен `WS_CLIPCHILDREN` (предотвращает мерцание при перерисовке mpv-кадров) и `WS_POPUP` в маске очистки (не все QQuickWindow создаются с одинаковым начальным стилем).
  - В `GWL_EXSTYLE` добавлена очистка `WS_EX_OVERLAPPEDWINDOW` (раньше оставался от Qt — создавал двойную рамку у child-окна).
  - После установки стилей — `SetWindowPos(… SWP_FRAMECHANGED …)`, который принудительно пересчитывает non-client area и применяет изменения стилей.
  - В `detach()` порядок обратный: сначала восстанавливаем стили, потом `SetParent` обратно, потом `SetWindowPos(SWP_FRAMECHANGED)`.
- **Правка `src/core/WallpaperEngine.{h,cpp}` — render pump:**
  - Добавлен `QTimer m_renderPumpTimer` (интервал 33 мс ≈ 30 Гц) в `WallpaperEngine`. Запускается в `attach()`, останавливается в `detach()`.
  - Тик зовёт `m_window->update()`, что ставит paint-job в очередь scene graph. Это компенсирует отсутствие `QExposeEvent` у child-окна WorkerW: даже если платформа не считает окно «exposed», scene graph с `setPersistentSceneGraph(true)` всё равно прогоняет сцену по таймеру.
  - 30 Гц — компромисс: достаточно для плавного видео (mpv всё равно рендерит через свой update-callback на каждом новом кадре), и не создаёт заметной нагрузки на CPU. Фактический FPS определяется `fpsLimit` в настройках (по умолчанию 60), а pump — это страховка от полного замирания.
- **Почему превью в приложении работало, а обои — нет:** `PreviewArea` живёт внутри главного `ApplicationWindow`, которое является нормальным top-level окном. Qt получает `QExposeEvent` для него штатно, scene graph рисуется без проблем, MpvObject рендерит. Хост-окно обоев после `SetParent` к WorkerW — уже child-окно, и expose flow ломается.
- **Затронутые файлы:** `src/qml/pages/HomePage.qml`, `src/qml/Main.qml`, `src/core/WallpaperEngine.h`, `src/core/WallpaperEngine.cpp`.
